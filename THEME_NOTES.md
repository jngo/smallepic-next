...
- `src/components/ui/menubar.tsx`: raised dropdown content above desktop windows and pins menu backgrounds to the popover token so surfaces remain readable.
- `src/components/ui/window.tsx`: isolates and clips windows, with the existing card surface applied to content panes so overlapping windows do not visually bleed through.
