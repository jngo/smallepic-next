...
export default function Desktop({ initialPathSegments }: DesktopProps) {
  ...
  const showWindow = useCallback((windowId: WindowKey, origin: string) => {
    ...
    const chain = getWindowChain(windowId);
    setWindows(prev => {
      const next = { ...prev };
      chain.forEach((key) => {
        next[key] = true;
  ...
  const closeWindow = useCallback((windowId: WindowKey) => {
    track("window_close", { id: windowId });
    const keysToClose = getWindowAndDescendantWindowKeys(windowId);
